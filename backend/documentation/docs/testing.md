...
## End-to-End Strategy
- Use Docker dev stack (`docker-compose.dev.yml`) for full-stack checks.
- Seed test data by creating a client via `/api/v1/accounts`, then walk through onboarding and payment flows.
- Keep a dedicated Stripe test connected account for replaying webhooks consistently.
- Record any manual test results in `manual_checks.md` with dates and outcomes.
